...
def generate_article(title_en: str, body_en: str) -> tuple[str, str, str, list[str]]:
    ...
    # ---- 記事本文生成 ----
    article_prompt = f"""あなたはAI経済新聞の編集デスクです。
一次情報をもとに、転載ではなく「AI産業の構造理解」に変換する日本語記事を生成してください。
...
【絶対ルール】
・元記事の見出し、リード、段落構成をコピーしない
・本文転載、長文引用、言い換えだけの記事化は禁止
・一次情報が示す業界構造、供給網、投資関係、API競争、GPU依存、クラウド基盤、モデル競争を解説する
・1行目: タイトル（32〜42文字）
...
・出典は一次情報として扱い、記事本文ではその発表がどの産業レイヤーに効くかを中心に書く"""
